...
void WebSockets::handleWebsocket(WSclient_t * client)
{
    ...
    WSopcode_t opCode = (WSopcode_t) (buffer[0] & 0x0F);

    bool mask = ((buffer[1] >> 7) & 0x01);
    size_t payloadLen = (WSopcode_t) (buffer[1] & 0x7F);
...
#ifndef NODEBUG_WEBSOCKETS
    bool fin = ((buffer[0] >> 7) & 0x01);
    bool rsv1 = ((buffer[0] >> 6) & 0x01);
    bool rsv2 = ((buffer[0] >> 5) & 0x01);
    bool rsv3 = ((buffer[0] >> 4) & 0x01);
...
bool WebSockets::readWait(WSclient_t * client, uint8_t *out, size_t n)
{
    ...
    while(n > 0) {
		...
		t = millis();
		size_t len = client->tcp->read((uint8_t*) out, n);
		out += len;
		n -= len;
    }
    return true;
